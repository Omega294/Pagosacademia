    ...
    def do_POST(self):
        if self.path == '/api/data':
            ...
            try:
                ...
                print("✓ Datos guardados localmente con éxito en tournament_data.json")
                self.send_response(200)
                self.send_header('Content-type', 'application/json; charset=utf-8')
                self.end_headers()
...
def run():
    # Change working directory to the directory of the script to serve its files
    ...
    while port < PORT + 10:
        try:
            # Reusable port setting
            socketserver.TCPServer.allow_reuse_address = True
            with socketserver.TCPServer(("", port), CustomHandler) as httpd:
                print(f"\n========================================================")
                print(f"  SERVIDOR LOCAL SOFTBALL TRACKER ACTIVO")
